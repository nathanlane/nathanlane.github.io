...
files.forEach(file => {
  ...
  try {
    ...
    if (!data.title) {
      ...
      fileErrors.push(`Title too long (${data.title.length}/${MAX_TITLE_LENGTH} chars): "${data.title}"`);
    }
    ...
    if (!data.description) {
      ...
      if (data.description.length < MIN_DESC_LENGTH) {
        fileErrors.push(`Description too short (${data.description.length}/${MIN_DESC_LENGTH} chars)`);
      }
      if (data.description.length > MAX_DESC_LENGTH) {
        fileErrors.push(`Description too long (${data.description.length}/${MAX_DESC_LENGTH} chars)`);
      }
