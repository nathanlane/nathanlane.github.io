...
/**
 * Jekyll to Astro Migration Script - Customized for nathanlane.github.io archive
 * 
 * Usage: node migrate-jekyll-archive.js
 */
...
function convertFrontmatter(jekyllFrontmatter, filename) {
  ...
  astroFrontmatter.tags = [...new Set(astroFrontmatter.tags.map(tag => 
...
function processImages(content, filename) {
  ...
  imagePaths.forEach(({ original }) => {
    ...
    const newPath = original.replace('/uploads/', '/images/blog/uploads/')
...
function migratePost(filePath) {
  ...
  // Convert frontmatter
  let astroFrontmatter = convertFrontmatter(frontmatter, filename);
  ...
  let astroContent = processedContent;
