...
export async function runSEOAudit() {
  ...
  posts.forEach((post) => {
    ...
    if (title.length > 60) {
      warnings.push(
        `Post "${title}" has title longer than 60 chars (${title.length})`,
      );
    }
    ...
    if (description.length < 120) {
      warnings.push(
        `Post "${title}" has short description (${description.length} chars) - aim for 120-160`,
      );
    }
    if (description.length > 160) {
      issues.push(
        `Post "${title}" has description over 160 chars (${description.length}) - bad for SEO`,
      );
    }
  ...
  research.forEach((item) => {
    ...
    if (description.length > 300) {
      warnings.push(
        `Research "${title}" has very long description (${description.length} chars)`,
      );
    }
  });
  ...
  projects.forEach((project) => {
    ...
    if (description.length < 120) {
      warnings.push(
        `Project "${title}" has short description (${description.length} chars)`,
      );
    }
  });
