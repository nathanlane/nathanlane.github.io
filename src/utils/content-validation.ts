...
export async function validateSlugUniqueness() {
	...
	const collections = ["post", "note", "research", "projects", "writing"] as const;
...
export async function validateSEOFields() {
	...
	posts.forEach((post) => {
		if (!post.data.description || post.data.description.length < 50) {
			errors.push(`Post "${post.data.title}" has description shorter than 50 characters`);
		}
		if (post.data.description && post.data.description.length > 160) {
			errors.push(`Post "${post.data.title}" has description longer than 160 characters`);
		}
	});
