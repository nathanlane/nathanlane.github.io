...
The web has conventions for good reasons. When we work with them rather than against them, everyone benefits—screen reader users, search engines, and developers who have to maintain the code.---*Technical note: This approach uses Tailwind's `@apply` directive to map utility classes to semantic elements. The `:not([class])` selector ensures we only affect plain heading tags, preserving the ability to override with explicit classes when needed.*
