...
const post = defineCollection({
  ...
  schema: ({ image }) =>
    baseSchema.merge(seoSchema).merge(slugSchema).extend({
      coverImage: z
        .object({
          alt: z.string(),
          src: image(),
...
const note = defineCollection({
  ...
  schema: baseSchema.merge(slugSchema).extend({
...
const research = defineCollection({
  ...
  schema: researchBaseSchema.merge(slugSchema).merge(seoSchema.pick({ ogImage: true, canonical: true, lang: true })).extend({
...
const writing = defineCollection({
  ...
  schema: baseSchema.merge(seoSchema).merge(slugSchema).extend({
    publishDate: z
      .string()
      .or(z.date())
      .transform((val) => new Date(val)),
