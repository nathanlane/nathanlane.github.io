	...
	try {
		...
		if (frontmatter.description) {
			if (frontmatter.description.length < 20) {
				console.log(`⚠️  ${file}: Description too short (${frontmatter.description.length} chars)`);
			} else if (frontmatter.description.length > 300) {
				console.log(`⚠️  ${file}: Description too long (${frontmatter.description.length} chars)`);
			}
		} else {
