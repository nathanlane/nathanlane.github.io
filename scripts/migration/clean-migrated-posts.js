...
function convertHtmlImages(content) {
  ...
  const divImagePattern = /<div\s+class=["']media\s+image["'][^>]*>[\s\S]*?<img\s+[^>]*src=["']([^"']+)["'][^>]*\/?>[\s\S]*?<\/div>/gi;
  ...
  try {
    ...
    if (content !== originalContent) {
      ...
      if (dryRun) {
        ...
        console.log('  - Jekyll vars fixed:', (content.match(/\{\{/g) || []).length < (originalContent.match(/\{\{/g) || []).length);
        console.log('  - HTML images converted:', content.includes('<img') < originalContent.includes('<img'));
        console.log('  - P tags removed:', (content.match(/<p>/g) || []).length < (originalContent.match(/<p>/g) || []).length);
      }
    } else {
