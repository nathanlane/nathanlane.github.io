...
files.forEach(file => {
  try {
    ...
    if (data.title && data.title.length > 60) {
      issues.longTitles.push({ file, title: data.title, length: data.title.length });
    }
    ...
    if (!data.description) {
      ...
      if (data.description.length < 20) {
        issues.shortDescriptions.push({ file, desc: data.description, length: data.description.length });
      }
      if (data.description.length > 300) {
        issues.longDescriptions.push({ file, desc: data.description.substring(0, 50) + '...', length: data.description.length });
      }
      if (data.description.includes('<') || data.description.includes('>')) {
        issues.htmlInDescription.push({ file, desc: data.description.substring(0, 50) + '...' });
      }
    }
  } catch (error) {
