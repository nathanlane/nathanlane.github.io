...
async function extractURLs(sitemapPath) {
	try {
		...
		urls.forEach((url) => {
			...
			} else if (url.path.includes("/portfolio/") || url.path.includes("/work/")) {
				patterns.portfolio.push(url);
			} else if (url.path.includes("/products/") || url.path.includes("/shop/")) {
		...
		const outputPath = path.join(path.dirname(sitemapPath), "squarespace-urls.json");
