...
// Configuration
const JEKYLL_POSTS_DIR =
  "/Users/nathanlane/code/nathanlane_github_io_archive/_posts";
const JEKYLL_ASSETS_DIR = "/Users/nathanlane/code/nathanlane_github_io_archive";
const ASTRO_CONTENT_DIR =
...
function migratePost(filePath) {
  ...
  // Process images
  const { content: processedContent, images } = processImages(
    content,
    filename,
  );
...
async function migrate() {
  ...
  try {
    ...
    console.error(
      "❌ Please install gray-matter first: npm install gray-matter",
    );
    process.exit(1);
  }
