    ...
    if (!data.draft) {
      ...
      if (!data.description) {
        ...
        if (data.description.length < 120) {
          warnings.push(`Post "${data.title}" has short description (${data.description.length} chars) - aim for 120-160`);
        }
        if (data.description.length > 160) {
          issues.push(`Post "${data.title}" has description over 160 chars (${data.description.length}) - bad for SEO`);
        }
      }
    }
  });
  ...
  if (fs.existsSync(researchDir)) {
    ...
    researchFiles.forEach(file => {
      ...
      if (data.description && data.description.length > 300) {
        warnings.push(`Research "${data.title}" has very long description (${data.description.length} chars)`);
      }
    });
  }
  ...
  if (fs.existsSync(projectsDir)) {
    ...
    projectFiles.forEach(file => {
      ...
      if (data.description && data.description.length < 120) {
        warnings.push(`Project "${data.title}" has short description (${data.description.length} chars)`);
      }
    });
  }
